 ...
 * cross-border transfers, legacy Paga payouts). One DataTable whose
...
const pagaColumns: Array<Column<Row>> = [
  { key: "created_at", header: "Time", cell: (r) => <span className="whitespace-nowrap">{fmtMs(s(r.created_at))}</span> },
  { key: "user_id", header: "User", cell: (r) => textCell(r.user_id) },
  {
    key: "usdsui_amount",
  ...
  },
  {
    ...
    header: "NGN",
    align: "right",
    cell: (r) => <span className="font-mono text-fg-muted">{fmtCcy(s(r.ngn_amount), "NGN")}</span>,
  },
  ...
  { key: "paga_reference", header: "Reference", cell: (r) => <CopyText value={s(r.paga_reference)} display={shortHash(s(r.paga_reference), 8, 4)} /> },
...
function columnsFor(source: Source): Array<Column<Row>> {
  ...
  if (source === "paga") return pagaColumns;
...
export default function TransactionsPage() {
  ...
  return (
    <div>
      <SectionHeader
        title="Transactions"
        subtitle="Successful, pending and failed across on-chain history, cross-border transfers and Paga payouts."
        right={
          <button
            type="button"
            onClick={refetch}
  ...
  return (
    <Drawer open={!!row} onClose={onClose} title={title}>
      <div className="space-y-4">
        ...
        {source === "paga" ? <PagaDetail row={row} /> : null}
      </div>
    </Drawer>
  );
}
...
function PagaDetail({ row }: { row: Row }) {
  return (
    <>
      <Field label="ID">
        <CopyText value={s(row.id)} display={shortHash(s(row.id), 8, 6)} />
      ...
      <Field label="User ID">{detailVal(row.user_id)}</Field>
      <Field label="USDsui">{fmtUsd(s(row.usdsui_amount))}</Field>
      <Field label="NGN amount">{fmtCcy(s(row.ngn_amount), "NGN")}</Field>
      <Field label="FX rate">{detailVal(row.fx_rate)}</Field>
      ...
      <Field label="Paga reference">
        <CopyText value={s(row.paga_reference)} display={shortHash(s(row.paga_reference), 8, 4)} />
      </Field>
      <Field label="Status">
