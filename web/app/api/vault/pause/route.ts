...
/**
 * POST /api/vault/pause
 *
 * Builds a PTB calling `auto_swap::pause<T>(&mut cap)` — flips the cap's
 * `paused` flag to true so the worker's `validate_for_swap` aborts.
 *
 * Body: { capId, sourceType }
 * Returns: { bytesB64, sender }
 *
 * Note: we deliberately don't fetch the cap object on chain to verify it
 * matches `sourceType`. The PTB itself will fail to build / execute if
 * the type-args mismatch, and re-doing the read here would just double
 * the latency.
 */
export async function POST(req: Request) {
  ...
  try {
    const tx = buildPauseAutoSwapTx(user.sui_address, capId, sourceType);
