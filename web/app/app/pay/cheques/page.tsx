  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="space-y-5 p-5">
        ...
        <Field label="Pay to (name on the cheque)">
          <input
            value={payee}
            onChange={(e) => setPayee(e.target.value)}
            maxLength={80}
            placeholder="e.g. Sele"
            className="w-full border-b border-line bg-transparent pb-2 text-[15px] text-fg outline-none placeholder:text-fg-dim"
          />
        </Field>
        ...
        >
          <span className="flex flex-col">
            <span className="text-[14px] text-fg">Restrict by country</span>
...
function IssuedView({
  ...
  const reclaim = useCallback(async () => {
    ...
    try {
      ...
      setError(friendlyError(e, "Couldn't claim this cheque back right now.", "Cheques"));
    } finally {
      setReclaiming(false);
    }
      ...
      {!reclaimed && (
        <>
          ...
          <button
            type="button"
            onClick={copy}
            className="talise-glass mx-auto flex w-full max-w-md items-center gap-2 px-4 py-3 text-left"
  ...
  if (cashed != null) {
    return (
      <div className="flex flex-col items-center gap-4 py-8 text-center">
        ...
        <h2 className="font-display text-[20px] font-medium text-fg" style={{ letterSpacing: "-0.02em" }}>
          Cashed
        </h2>
        <p className="text-[14px] text-fg-muted">It&apos;s in your Talise balance.</p>
        <PrimaryButton
          onClick={() => {
            setCashed(null);
            setPreview(null);
  ...
  if (preview && parsed) {
    return (
      <div className="space-y-5 text-center">
        <p className="text-[14px] text-fg-muted">From {preview.creatorDisplay}</p>
        <ChequeCard
  ...
  return (
    <div className="space-y-5">
      <GlassCard className="space-y-4 p-5">
        <Field label="Paste a cheque link" hint="It looks like talise.io/c/…">
          <textarea
            value={link}
...
function MineTab({
  ...
  return (
    <div className="space-y-3">
      {rows.map((row) => (
        <GlassCard key={row.id} className="space-y-3 p-4" radius={18}>
          <div className="flex items-start justify-between gap-3">
            <AmountDisplay usd={row.amountUsd} size={22} subAsset />
            <StatusPill label={row.status} tone={statusTone(row.status)} />
          </div>
          {subtitleFor(row) && (
            <p className="text-[13px] text-fg-muted">{subtitleFor(row)}</p>
          )}
          <p className="font-mono text-[10px] text-fg-dim">{dateText(row.createdAt)}</p>
          {row.reclaimable && (
            <PrimaryButton
              variant="ghost"
              full
...
function gateError(e: unknown): string {
  if (e instanceof ApiError) {
    ...
    if (e.code === "NOT_SIGNED_IN") return "Taking you to sign in…";
