...
export function ScanSheet({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black">
      ...
      {/* Dim scrim with viewfinder cut-out */}
      <div className="absolute inset-0 flex items-center justify-center">
        <div
          className="relative size-64 rounded-[28px]"
          style={{ boxShadow: "0 0 0 200vmax rgba(0,0,0,0.45)" }}
        >
          ...
          {(["-top-0 -left-0 border-t-[3px] border-l-[3px] rounded-tl-[28px]",
            ...
            <span
              key={cls}
              className={`absolute size-12 border-[#cbf3b4] ${cls}`}
              aria-hidden
            />
          ))}
          {/* Sweep line */}
          {cam === "scanning" && (
            <span
              ...
              style={{
                background:
                  "linear-gradient(90deg, transparent, rgba(203,243,180,0.95), transparent)",
                boxShadow: "0 0 12px rgba(203,243,180,0.55)",
              }}
            />
          )}
        </div>
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent px-8 pb-12 pt-12 text-center">
        {cam === "denied" ? (
          ...
          <p className="inline-block rounded-full bg-white/15 px-4 py-2 text-[13px] text-white">
