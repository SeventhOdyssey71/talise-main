...
export function AmountDisplay({
  ...
  return (
    <span className={`inline-flex flex-col ${className}`}>
      <span
        className={`font-display font-semibold tabular-nums ${muted ? "text-fg-muted" : "text-fg"}`}
        style={{ fontSize: size, lineHeight: 1.04, letterSpacing: size >= 30 ? "-0.03em" : "-0.02em" }}
      >
        {text}
      </span>
      {subAsset && (
        <span className="mt-1 font-mono text-[11px] text-fg-dim tabular-nums">{usdsui}</span>
      )}
    </span>
  );
}
